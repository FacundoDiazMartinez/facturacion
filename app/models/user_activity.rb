class UserActivity < ApplicationRecord
  ...
  def self.create_for_confirmed_invoice invoice  #Se ejecuta cuando una factura se confirma
  	UserActivity.create(
        ...
        title: "El usuario #{invoice.user.name} emitió un comprobante",
        body: "El día #{I18n.l(Date.today)} generó y confirmó un comprobante tipo #{invoice.tipo} destinado al cliente #{invoice.client.name}, por un monto de $#{invoice.total.round(2)}."
    )
  end
  ...
  def self.create_for_approved_user user #Se ejecuta cuando se aprueba un usuario
  	UserActivity.create(
        ...
        body: "El día #{I18n.l(Date.today)} se le dio acceso a #{user.name} para hacer uso de las funciones del sistema de #{user.company.name}."
    )
  end
  ...
  def self.create_for_new_client client #Se ejecuta cuando se crea un cliente
    UserActivity.create(
        ...
        body: "El día #{I18n.l(Date.today)} el usuario #{client.user.name} registró al cliente #{client.name}."
    )
  end
  ...
  def self.create_for_sended_purchase_order purchase_order #Se ejecuta cuando se envia una ord. de compra al proveedor
    UserActivity.create(
        ...
        body: "El día #{I18n.l(Date.today)} el usuario #{purchase_order.user.name} envio al proveedor #{purchase_order.supplier.name} la orden de compra Nº #{purchase_order.number}."
    )
  end
  ...
  def self.create_for_purchase_order purchase_order
    UserActivity.create(
        ...
        body: "El día #{I18n.l(Date.today)} el usuario #{purchase_order.user.name} actualizó la orden de compra Nº #{purchase_order.number} a '#{purchase_order.state}'."
    )
  end

  def self.create_new_product product
      UserActivity.create(
  ...
  def self.create_for_product_price_history product_price_history #Se crea cuando el precio del producto cambia
    ...
    if activities.count > 1 #si existe actualiza la ultima y la hace mas general
      activities.last.update(
        ...
        body: "El día #{I18n.l(Date.today)} el usuario #{product_price_history.user.name} actualizó el precio de #{activities.count} productos."
      )
      ...
      UserActivity.create(
          ...
          body: "El día #{I18n.l(Date.today)} el usuario #{product_price_history.user.name} actualizó el precio de #{product_price_history.product.name} de $#{product_price_history.old_price.round(2)} a $#{product_price_history.price.round(2)}."
      )
    end
  end
  ...
  def self.create_for_arrival_note arrival_note #Se crea cuando se registra un remito de llegada de mercaderia
    UserActivity.create(
        ...
        body: "El día #{I18n.l(Date.today)} el usuario #{arrival_note.user.name} registró un remito de la orden de compra Nº#{arrival_note.purchase_order.number} del proveedor #{arrival_note.purchase_order.suppler.name} p."
    )
  end
  ...
  def self.create_for_purchase_invoice purchase_invoice #Se crea cuando se carga una factura al sistema de un proveedor
    UserActivity.create(
        ...
        body: "El día #{I18n.l(Date.today)} el usuario #{purchase_invoice.user.name} registró la factura Nº#{purchase_invoice.number}."
    )
  end
end
