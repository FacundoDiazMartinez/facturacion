class Payments::ChequePaymentsController < Payments::PaymentsController
  ...
  def index
    @cheque_payments = current_company.cheque_payments.joins(:payment).where(payments: { confirmed: true }).includes(:payment).search_by_number(params[:number]).search_by_client(params[:client]).search_by_date(params[:date]).order("cheque_payments.created_at DESC").paginate(page: params[:page], per_page: 10)
  end
