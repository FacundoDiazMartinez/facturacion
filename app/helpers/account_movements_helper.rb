	...
	def comprobante_con_link(account_movement)
		if not account_movement.invoice_id.nil?
			...
			if account_movement.cbte_tipo == "Devolución"
				...
				link_to "#{(account_movement.cbte_tipo=="00"? "X" : account_movement.cbte_tipo[-1,1])} - #{account_movement.receipt.nil? ? "" : account_movement.receipt.sale_point.name} - #{account_movement.receipt.nil? ? "" : account_movement.receipt.number}", edit_receipt_path(account_movement.receipt_id)
