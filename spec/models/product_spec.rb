...
RSpec.describe Product, type: :model do
  ...
  it { should validate_uniqueness_of(:code).scoped_to([:company_id, :active, :tipo]).with_message('Ya existe un producto/servicio con el mismo identificador.') }
  it { should validate_presence_of(:name).with_message('El nombre del producto no puede estar en blanco.') }
  it { should validate_uniqueness_of(:name).scoped_to([:company_id, :active]).with_message('Ya existe un producto/servicio con el mismo nombre.') }
